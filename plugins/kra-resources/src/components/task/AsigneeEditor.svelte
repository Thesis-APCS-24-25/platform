...
<script lang="ts">
  ...
  type AssigneeObject = (Doc | any) & Pick<WeeTask, 'space' | 'assignee' | 'identifier'>
  ...
  function getCategories (object: AssigneeObject | AssigneeObject[]): void {
    ...
    if (cdocs.length > 0) {
      categories.push({
        label: kra.string.PreviousAssigned,
        func: async () => {
          const r: Ref<Person>[] = []
          for (const d of cdocs) {
            r.push(...(await getPreviousAssignees(d._id as Ref<WeeTask>)))
          }
          return r
        }
      })
    ...
    categories.push({
      ...
      func: async () => {
        ...
        const projects = await client.findAll(kra.class.Project, {
...
{#if _object}
  ...
  {:else}
    <AssigneeBox
      ...
      placeholder={kra.string.Assignee}
      value={sel}
      {categories}
      titleDeselect={kra.string.Unassigned}
      {size}
      {kind}
      {avatarSize}
      {width}
      ...
      justify={'left'}
      showTooltip={{
        label: kra.string.AssignTo,
        personLabel: kra.string.AssignedTo,
        placeholderLabel: kra.string.Unassigned,
